...
def mask_tokens(inputs, tokenizer,pos_ids,\
    special_tokens_mask=None, mlm_probability=0.15):
        ...
        # probability_matrix[torch.where(pos_ids==14)] = 0.1
...
class view_generator:
    ...
    def random_token_replace(self, ids,pos_ids):
        mask_id = self.tokenizer.convert_tokens_to_ids(self.tokenizer.mask_token)
        ids, _ = mask_tokens(ids, self.tokenizer,pos_ids, mlm_probability=0.25)
