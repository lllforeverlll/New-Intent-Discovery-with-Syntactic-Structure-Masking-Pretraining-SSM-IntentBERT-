...
def find_farthest_point(data, center):
    distances = np.linalg.norm(data - center, axis=1)
    farthest_index = np.argmin(distances)
    return farthest_index, distances[farthest_index]
class MemoryBank(object):
    ...
    def mine_nearest_neighbors(self, topk, calculate_accuracy=True):
        ...
        for i in range(0,n):
            ...
            center_mean = np.mean(data, axis=0)  # 假设质心为数据的均值
            farthest_index, farthest_distance = find_farthest_point(data, center_mean)
            # print(farthest_index)
            
#             --------------------------------------------
            # indices[i][topk+1] = indices[i][farthest_index]

            # print(indices[i][farthest_index])
