...
def makenv(data):
    ...
    datas=[]
    key = ['tword','txpos', 'tupos', 'tdeprel']
    a = dict([(k, []) for k in key])
    file_name = 'bankingpos.json'
    ...
    for i,d in enumerate (data):
        ...
        for sent in doc.sentences :
            for word in sent.words:
                ...
                a["txpos"].append(word.xpos)
                a["tupos"].append(word.upos)
                a["tdeprel"].append(word.deprel)
            datas.append(a)
            a = dict([(k, []) for k in key])
...
class IntentDataset():
    ...
    def loadDataset(self, dataDirList):
        ...
        dataFilePathList = \
            [os.path.join('IntentBert-main/data', dataDir, FILE_NAME_DATASET) for dataDir in dataDirList]
