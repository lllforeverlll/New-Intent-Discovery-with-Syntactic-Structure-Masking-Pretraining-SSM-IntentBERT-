...
def makeTrainExamples(data:list, tokenizer, label=None, mode='unlabel'):
    ...
    if mode != "unlabel":
        ...
        examples = tokenizer.pad(data, padding='longest', return_tensors='pt')
        dataFilePath="IntentBert-main/cixing.json"
...
def mask_tokens1(inputs, tokenizer,cixing,\
    special_tokens_mask=None, mlm_probability=0.0):
        """
...
#       将特殊字符的地方全部置为0
        ...
        probability_matrix[torch.where(cixing==14)] = 0.3
...
def mask_tokens2(inputs, tokenizer,cixing,\
    special_tokens_mask=None, mlm_probability=0.0):
        ...
        # probability_matrix[torch.where(cixing==10)] = 0.15


        # probability_matrix[:, 0:6].fill_(0.0)
        # probability_matrix[:, 6:22].fill_(0.0)
        masked_indices = torch.bernoulli(probability_matrix).bool()
        labels[~masked_indices] = -100  # We only compute loss on masked tokens
...
def mask_tokens3(inputs, tokenizer,cixing,\
    special_tokens_mask=None, mlm_probability=0.2):
        """
        Prepare masked tokens inputs/labels for masked language modeling: 80% MASK, 10% random, 10% original.
        """
        labels = inputs.clone()
        ...
        probability_matrix[torch.where(cixing==14)] = 0
        probability_matrix[torch.where(cixing==13)] = 0
        # probability_matrix[torch.where(cixing==10)] = 0.2
        probability_matrix[:, 10:45].fill_(0.0)

        
        
        # probability_matrix[:, 6:23].fill_(0.0)
...
def mask_tokens4(inputs, tokenizer,cixing,\
    special_tokens_mask=None, mlm_probability=0.2):
        ...
        probability_matrix[torch.where(cixing==14)] = 0
        probability_matrix[torch.where(cixing==13)] = 0
#         probability_matrix[torch.where(cixing==10)] = 0
        probability_matrix[:, 0:9].fill_(0.0)
